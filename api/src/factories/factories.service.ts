...
export class FactoriesService {
    correios(response: any) {
        ...
        for (const evento of response.eventos) {
            ...
            if (evento.subStatus && (evento.subStatus[0].startsWith("Registrado") || (evento.subStatus[0].startsWith("de") || evento.subStatus[0].startsWith("Origem"))))
            ...
            registroMovimentacoes.push({
                data_movimentacao: DateTime.fromFormat(evento.data, 'dd/MM/yyyy').toISODate(),
